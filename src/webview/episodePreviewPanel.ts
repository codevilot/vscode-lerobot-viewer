...
export class EpisodePreviewPanelManager implements vscode.Disposable {
  ...
  async showMetadata(datasetId: string): Promise<void> {
    ...
    if (this.panel) {
      ...
      this.panel.showMetadata(metadata);
      this.panel.reveal();
    } else {
...
class EpisodePreviewPanel extends BaseWebviewPanel {
  ...
  showMetadata(metadata: DatasetMetadataView): void {
    this._metadataMode = true;
    this.panel.title = `${metadata.descriptor.name}`;
    this.post({ type: "init-metadata", data: metadata });
  }
